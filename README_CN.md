...
**原则四——MCP 优先的工具层**
工具不是硬编码的。启动时，Agent 向每个配置的 MCP 服务器发送 `tools/list` 请求，动态加载工具 Schema。无需修改 Agent 代码即可增减工具服务器。
...
在 `mcp/servers.yaml` 中配置工具服务器，Agent 启动时自动发现所有工具。
...
Agent 在每个阶段只能看到对应的工具。处于文献精读阶段时无法调用 `execute_python`，处于实验阶段时无法调用 `arxiv_search`。这防止了能力泄漏，让模型保持专注。
